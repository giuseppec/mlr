#' @export
makeRLearner.classif.xgboost.hash = function() {
  makeRLearnerClassif(
    cl = "classif.xgboost.hash",
    package = "xgboost",
    par.set = makeParamSet(
      ...
      makeIntegerLearnerParam(id = "hash.size", default = 100, lower = 0),
    ...
    par.vals = list(nrounds = 100, missing = NA_real_, hash.size = 2^10),
...
#' @export
trainLearner.classif.xgboost.hash = function(.learner, .task, .subset, .weights = NULL,  ...) {
  td = getTaskDescription(.task)
  data = getTaskData(.task, .subset, target.extra = TRUE)
  target = data$target
  target = match(as.character(target), td$class.levels) - 1
  hash.size = getHyperPars(.learner)$hash.size
...
#' @export
predictLearner.classif.xgboost.hash = function(.learner, .model, .newdata, ...) {
  td = .model$task.desc
  m = .model$learner.model
  hash.size = getHyperPars(.learner)$hash.size
  dat = FeatureHashing::hashed.model.matrix( ~ . - 1, .newdata, hash.size = hash.size)
  cat(hash.size)
  p = xgboost::predict(m, newdata = dat, ...)
